...
function populate(table, is_disasm = false) {
    ...
    for (let i = 0; i < MEMCONF.region_size / COLUMNS; ++i) {
        ...
        if (is_disasm)
            for (let j = 0; j < COLUMNS && i * COLUMNS + j < MEMCONF.region_size; j += 4) {
            ...
            for (let j = 0; j < COLUMNS && i * COLUMNS + j < MEMCONF.region_size; ++j) {
                let cell = document.createElement('td');
...
export default class Memory
{
    ...
    constructor(simulator, table, pageid, selector) {
        ...
        this.page_label = typeof pageid === 'string' ? document.getElementById(pageid) : pageid;
    ...
    update() {
        ...
        const values =
            disasm ? this.simulator.disassemble_region(this.region_id) : this.simulator.read_region(this.region_id);
        let new_page = this.last_hash === null;
        this.last_hash = new_hash;

        for (let i = 0; i < MEMCONF.region_size / COLUMNS; ++i)
            this.table.rows[i + 1].cells[0].textContent =
                (this.region_id * MEMCONF.region_size + i * COLUMNS).toString(16).toUpperCase().padStart(8, '0');
