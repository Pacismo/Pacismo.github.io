    ...
    show() {
        return new Promise((res, rej) => {
            ...
            const conf = {
                ...
                /** @type {HTMLInputElement} */
                cache_instruction_enabled : document.getElementById('conf_cache_instruction_enabled'),
                /** @type {HTMLInputElement} */
            ...
            submit.onclick = async () => {
                try {
                    ...
                    const instruction_set_bits = Number.parseInt(conf.cache_instruction_setbits.value);
                    const instruction_offset_bits = Number.parseInt(conf.cache_instruction_offsetbits.value);
                    const instruction_ways = Number.parseInt(conf.cache_instruction_ways.value);

                    let files = [];

                    for (let i = 0; i < conf.asm.files.length; ++i)
                        files.push(new Input(conf.asm.files[i].name, await conf.asm.files[i].text()));
                    ...
                    let data_cache = new CacheConfiguration(data_enabled ? CacheMode.Associative : CacheMode.Disabled,
                                                            data_set_bits, data_offset_bits, data_ways);
                    let instruction_cache =
                        new CacheConfiguration(instruction_enabled ? CacheMode.Associative : CacheMode.Disabled,
                                               instruction_set_bits, instruction_offset_bits, instruction_ways);

                    this.configuration = new SimulationConfiguration(miss_penalty, volatile_penalty, writethrough,
                                                                     pipelining, instruction_cache, data_cache);
